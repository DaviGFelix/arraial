...
function formatarDiaSemana(valor, index = 0) {
    ...
    return data.toLocaleDateString(i18nLocale(), {
        weekday: 'short'
    }).replace('.', '');
}
...
function formatarNumero(valor, casas = 0) {
    ...
    return numero.toLocaleString(i18nLocale(), {
...
function formatarHora(valor) {
    ...
    return data.toLocaleTimeString(i18nLocale(), {
        hour: '2-digit',
        minute: '2-digit'
    });
}
...
function abrirModalReserva(passeioId) {
    ...
    if (!passeioSelecionado) {
        alert(i18nText('Passeio não encontrado.'));
        return;
    }
...
async function handleReservaSubmit(event) {
    ...
    if (!passeioSelecionado) {
        alert(i18nText('Nenhum passeio selecionado.'));
        return;
    }
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        alert(i18nText('Por favor, insira um e-mail válido.'));
        return;
    }
...
function buildWhatsAppLink(payload) {
    const dataFormatada = payload.data_passeio
        ? new Date(`${payload.data_passeio}T12:00:00`).toLocaleDateString(i18nLocale())
        : '-';
